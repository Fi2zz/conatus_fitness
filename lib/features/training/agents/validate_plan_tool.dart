...
class ValidatePlanTool extends Tool {
  ...
  String get description =>
      '提交训练计划 JSON 做结构校验与安全终审。完成计划后必须调用；'
      '校验通过后，把返回的 JSON 原样作为最终回复输出，不要附加任何文字。';

  @override
  List<ParamSpec> get params => <ParamSpec>[
        ParamSpec.string('plan_json', description: '完整的训练计划 JSON', required: true),
      ];

  @override
  Future<ToolResult> call(ToolContext context) async {
    final plan = TrainingPlanCodec.tryParse(context.str('plan_json'));
    if (plan == null) {
      const message = '计划未通过 JSON 结构校验：weeks/sessions/exercises '
