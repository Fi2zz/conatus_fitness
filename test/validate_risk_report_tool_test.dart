...
Future<ToolResult> submit(ValidateRiskReportTool tool, String json) => tool.call(
      ToolContext(
        ToolCall(name: ValidateRiskReportTool.toolName, arguments: {'report_json': json}),
      ),
    );
...
void main() {
  ...
  test('活跃伤病 + low → 通过但被安全改写', () async {
    final tool = ValidateRiskReportTool('膝盖旧伤');
    final low = validReport.replaceAll('"risk_level":"medium"', '"risk_level":"low"');
  ...
  test('伤病史非空但未识别风险部位 → 拦截', () async {
    final tool = ValidateRiskReportTool('膝盖旧伤');
    final empty = validReport.replaceAll('"risk_areas":["膝"]', '"risk_areas":[]');
